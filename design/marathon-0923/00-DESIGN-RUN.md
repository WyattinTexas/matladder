...
- 9/23 22:50 PT: **BUILD 8 ON TESTFLIGHT** — archived 22:45 (SPM pin widened), uploaded 22:46, VALID 22:48 (id 1bf0b034-9019-4fb1-a1d2-bf130479012a, 1.1 build 8), usesNonExemptEncryption PATCHed false, added to Friends 87221167… (+ Team). betaAppReviewSubmission REFUSED: ENTITY_UNPROCESSABLE.ANOTHER_BUILD_IN_REVIEW (build 6 WAITING_FOR_REVIEW since 17:45) → ~/.appstoreconnect/footwork-build8-autosubmit.py (nohup, log beside it) polls every 5 min and POSTs build 8 once the train is free; the build-7 loop was stopped (8 supersedes 7). NOT submitted to the App Store — the App Privacy label (Location · Precise · not linked · App Functionality) is Wyatt's click in ASC first. Web live: matladder.com + /app/ (6ab22e3 · b1828cc); drbango.com/ladder mirror pushed (DrBango 7dff6b5f; Pages redeploy pending at 22:50). The share image saved as stills-built/run-share.png (nit: its BEST tile reads "BEST MI"; the finish card says BEST MILE — next card). CARD-RUN DONE. 🔴 Wyatt: a real run outside (RUN → START → a mile → hear it, phone locked, music on) · the App Privacy label in ASC.
